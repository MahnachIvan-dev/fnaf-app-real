...
// ===== НАСТРОЙКИ ВРЕМЕНИ И ЭНЕРГИИ (1 ЧАС = 2 МИНУТЫ) =====
const HOUR_DURATION_MS = 2 * 60 * 1000; // 2 минуты = 1 час (ночь = 12 минут)
const MAX_POWER = 100;
const TICK_INTERVAL = 500;

// Расход в секунду (%)
const BASE_POWER_DRAIN = 0.050;   // Пассивный режим
const CAMERA_POWER_DRAIN = 0.040; // Планшет камер
const DOOR_POWER_DRAIN = 0.114;   // 2 закрытые двери = ~0.278%/сек -> ноль в 3 AM (6 минут)
...
setInterval(() => {
    ...
    Object.values(games).forEach(game => {
        ...
        // Время тикает ТОЛЬКО при включённом свете
        if (!game.systemOff) {
                ...
                if (game.settings.doorOverload && !game.overloadTripped) {
                    if (closedDoors >= 1) {
                        ...
                        if (game.overloadLevel >= 100) {
                            ...
                            game.power = Math.max(1, game.power - 3);
            ...
            // Время заморожено, тикает таймер гибели
...
io.on('connection', (socket) => {
    socket.on('createGame', (settings, cb) => {
        ...
        const links = {
            ...
            cameras: Array.from({length: 8}, (_, i) => `/camera.html?game=${game.id}&cam=${i}`)
        };
    ...
    socket.on('joinAsAnimatronic', ({ gameId, name }, cb) => {
        ...
        if (idx === -1) {
            idx = game.animatronics.length;
            game.animatronics.push({ connected: true, socketId: socket.id, name });
        } else {
    ...
    socket.on('toggleDoor', ({ gameId, doorIndex }) => {
        ...
        if (game && game.state === 'playing' && !game.systemOff && !game.overloadTripped) {
            ...
            setTimeout(() => {
                ...
                io.to('game_' + game.id).emit('doorAnimDone', { doorIndex, state: getPublicState(game) });
    ...
    socket.on('resetBreaker', (gameId) => {
        ...
        if (game && game.overloadTripped) {
            game.overloadTripped = false;
            game.overloadLevel = 0;
            io.to('game_' + game.id).emit('breakerReset', getPublicState(game));
        }
    });

    socket.on('playDistraction', ({ gameId }) => {
        io.to('game_' + gameId).emit('playDistractionSound');
    });
...
server.listen(PORT, '0.0.0.0', () => {
    console.log(`FNAF Server running on port ${PORT}`);
});
