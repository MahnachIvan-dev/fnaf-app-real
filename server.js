        ...
        if (!game.systemOff) {
            if (game.power > 0) {
                ...
                game.doors.forEach(d => {
                    if (d.closed) { drain += DOOR_POWER_DRAIN; closedDoors++; }
                });
                ...
                // ПЕРЕГРЕВ ДВЕРЕЙ: срабатывает при 1+ закрытой двери
...
io.on('connection', (socket) => {
    ...
    // ОТВЛЕКАЮЩИЙ ЗВУК (от аниматроника на камеры)
    ...
    // БЕЗЛИМИТНОЕ ОТКЛЮЧЕНИЕ ЭЛЕКТРИЧЕСТВА
