...
function getPublicState(game) {
    return {
        ...
        rebootApprovalNeeded: game.rebootApprovalNeeded, killPowerUsesLeft: game.killPowerUsesLeft,
        overloadLevel: Math.round(game.overloadLevel), overloadTripped: game.overloadTripped, doorOverloadEnabled: game.settings.doorOverload
    };
}
    ...
    Object.values(games).forEach(game => {
        ...
        if (!game.systemOff) {
            if (game.power > 0) {
                ...
                if (game.power <= 0) {
                    game.power = 0; game.systemOff = true; game.isDeadPower = true; game.camerasUp = false;
                ...
                if (game.settings.doorOverload && !game.overloadTripped) {
                    if (closedDoors >= 2) {
                        ...
                        if (game.overloadLevel >= 100) {
                            game.overloadLevel = 100; game.overloadTripped = true; game.power = Math.max(1, game.power - 3);
...
io.on('connection', (socket) => {
    ...
    socket.on('joinAsGuard', (gameId, cb) => {
        const game = games[gameId]; if (!game) return cb({ success: false });
        game.guardSocket = socket.id; socket.join('game_' + gameId); cb({ success: true, state: getPublicState(game) });
    });

    socket.on('joinAsCamera', ({ gameId, camIndex }, cb) => {
        const game = games[gameId]; if (!game || !game.cameras[camIndex]) return cb({ success: false });
        game.cameras[camIndex].connected = true; game.cameras[camIndex].socketId = socket.id; socket.join('game_' + gameId);
    ...
    socket.on('joinAsAnimatronic', ({ gameId, name }, cb) => {
        ...
        socket.join('game_' + gameId); cb({ success: true, animIndex: idx });
    });
    ...
    socket.on('killPower', ({ gameId }) => {
        ...
        if (game && game.state === 'playing' && !game.systemOff && game.killPowerUsesLeft > 0) {
            game.killPowerUsesLeft--; game.powerAtOut = game.power; game.systemOff = true; game.isDeadPower = false; 
    ...
    socket.on('disconnect', () => {
        if (socket.gameId && games[socket.gameId]) {
            if (socket.role === 'camera' && typeof socket.camIndex === 'number') { games[socket.gameId].cameras[socket.camIndex].connected = false; }
